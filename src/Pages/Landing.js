...
const Landing = () => {
  ...
  const fetchApi = async () => {
    ...
    formBody.append('id', pathname[1] !== 'consultation' ? '' : pathname[2])
    try {
      const fetchStat = await fetch('https://consultation19-server.herokuapp.com/covid19', {
      // const fetchStat = await fetch('http://localhost:8000/covid19', {
        method: 'POST',
        body: formBody
      }).then((res, err) => {
        setFetching(false)
          ...
          if (err || resJson.status === 'error') {
            ...
            Swal.fire({
              ...
              html: `${resJson.diagnosis.message}<br>
                        ${resJson.summary.qr && `<div>
                  <p className="card-text">Scan QR to see your history of covid symptom checks</p>
                  <p className="card-text">Save the image in your device if you wish to view it later</p>
                  <img src=${resJson.summary.qr} alt="" />
                </div>`}
